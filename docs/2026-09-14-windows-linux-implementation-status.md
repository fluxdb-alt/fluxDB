...
- 验证：connectors `190 passed / 0 failed / 16 ignored`（含新测试）；app `459 passed`；desktop `402 passed / 3 ignored`；`cargo check --workspace --locked` / fmt / `git diff --check` 通过。
- CI：run 35233654900（含本改动）三平台全绿（release 构建 + core/storage/app/connectors 测试）。
...
**CI 验证（run 35233654900，commit 0d4f14d 起含 TLS 继承，d93fd11 加打包）**：
- **Ubuntu 22.04**：`.deb` 构建 ✓ / 内容+解压校验 ✓ / SHA256 ✓ / ldd 依赖报告 ✓ / 上传 ✓（Windows 装器步骤按 `runner.os != 'Linux'` 正确跳过）。
- **Windows 2022**：choco 装 Inno Setup → ISCC 编译 `windows-installer.iss` ✓ / 校验存在 + SHA256 ✓ / 上传 ✓（Linux deb 步骤正确跳过）。
- **macOS**：不受 AI-05 影响，全绿。
