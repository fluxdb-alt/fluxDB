...
- **完成记录**：已完成；执行人 FluxDB；内容 —
  ...
  修复记录（2026-09-13）：更新路径行身份以跨 `::text` 比较，身份参数原按数字类型绑定，与服务端推断的 text 参数类型不符 → `error serializing parameter 0`，数据表更新任一整型主键行均失败；改为 `pg_text(value)` 文本绑定。`pg_identity_where_maps_null_value_to_is_null` 单测断言同步更新为文本比较形式；真库冒烟 `pg_live_smoke_typed_read_binary_and_apply_changes` 修复前失败、修复后通过。
  修复记录二（2026-09-13）：`pg_bind_sql` 的 SET/INSERT 值——网格把数值编辑成 `CellValue::F64` 后写 numeric 列会裸绑 `$n`（PG 推断 numeric → f64 无法序列化 → `error serializing parameter 0`），`apply_changes` ROLLBACK 前不返回 Err，表现为**网格保存静默不生效、无 Alert**。修复：非字符串/非整型/非 float 列任意标量一律 String 文本桥接 `CAST(CAST($n AS text) AS <type>)`（bytea 值仍原生 `Vec<u8>` 直绑不文本化）。回归：真库冒烟 `pg_live_smoke_numeric_edit_with_f64_value`（新增）+ `pg_live_smoke_typed_read_binary_and_apply_changes`（bytea 不回归）。另给 `pg_apply_changes` 补 begin/commit/aborted 三级日志（含表、操作计数、失败 `message=`），数据写操作可观测。
  修复记录三（2026-09-13，desktop 提示层）：网格「保存修改」确认后 dispatch 返回 `Failed` 时，该事件在 `confirm_apply_data_changes` 只被 `!matches!(Failed)` 判断用、**未进统一提示通道** → 保存失败静默。修复：`confirm_apply_data_changes` 对 `Failed` 调 `show_message(Error)`，走**底部居中 toast**（`app_message_overlay`，gpui-component Alert + 自动消失），与备份/用户管理保存失败一致。验证：用户故意输非法 timestamptz `+ss` → `apply_changes` ROLLBACK → 底部 toast 显示「保存失败：invalid input syntax for timestamp...」。
...
- **完成记录**：进行中；执行人 FluxDB（T12 增量二：dollar-quote 分句）；内容 — desktop `split_statements`（字节）+ `split_statement_ranges_snapshot`（snapshot）+ app `sql_statement_ranges` 均识别 PG `$$...$$` 与 `$tag$...$tag$` 并跳过体内分号/引号，`$1` 参数与 `$name` 识别失败不计开启（`::` 冒号本来就不触发）；`E'...'` 由既有全局反斜杠转义覆盖无需特判。验证 — 新增 desktop 3 测（字节/m命名标签+参数/snapshot）+ app 2 测（`sql_text_statement_ranges`），工作区全量通过（359+126+81+366+231…）；PG 实时冒烟 10 通过（顺带修正 T11 冒烟测试 `inserts` 1 行 vs 意图 3 行的测试数据口径）。剩余：`::`/`$n` 与 snippet tabstop 消歧、格式化保持函数体，另增增量。
  修复记录（2026-09-13，app `sql_format::sql_text_for_execution`）：下发的双引号归一化 `normalize_double_quoted_sql_strings`（`"..."`→`'...'`）原为全方言无条件执行——MySQL 语义正确，但 **PG 的 `"` 是标识符引用**，会把用户输入的 `UPDATE "public"."orders" SET "amount" = 684 WHERE "id" = 1` 改成 `UPDATE 'public'.'orders' SET 'amount' = ...`，服务端报 `syntax error at or near "'public'"`（日志 `PostgreSQL 执行失败 kind=Query`，SELECT+UPDATE 逐段各失败一次）。修复：`sql_text_for_execution` 加 `db_kind` 参数，仅 MySQL/TiDB/SQLite 归一，PG/Redis/Mongo 保留 `"`；5 处执行入口按 `connection_kind` 传入、未知回退 MySql。回归：app 单测 `execute_query_preserves_double_quoted_identifiers_for_postgres`；连接器 executor 失败日志补 `message=`+`sql=`。
...
- **完成记录**：已完成；执行人 FluxDB（T13 增量一~三）；内容 —
  ...
  **剩余（非必要）**：结果流式/有界存储（当前按结果集整体 `query()` 读取，10000 行导出走独立 `pg_export_pages` 一致快照路径；普通查询结果集内存有界属可选优化，非设计硬性缺失）、statement→result 索引（当前 results 与 summaries 按序一一对应，已足够）。
  修复记录（2026-09-14，T13 增量四：人工清单 D 组「查询取消/事务恢复」）：
  - **D1 停止按钮不生效**：桌面工具栏「停止」按钮此前没有任何点击处理，且 app 执行入口 `execute_query` 等价 `should_cancel = || false`——取消标志在 UI→app→connector 之间根本没有通路。新增 `AppCommand::CancelQueryExecution`/`AppEvent::QueryCancelRequested` 与 `AppController::query_cancel_flags`（`Arc<Mutex<BTreeMap<TabId, Arc<AtomicBool>>>>`：`StartQueryExecution` 登记新标志、关标签释放并置位、登记时清理已关标签），执行改走 `execute_query_with_cancel` → `execute_query_for_connection_with_progress` 下传标志；工具栏「停止」置位并提示「已请求停止执行」。（标志放共享 `Arc` 而非 `AppState`：UI 线程与后台执行线程各持一份 `AppController` 副本。）
  - **D2 取消如实收尾**：`pg_run_statement` 回传取消来源（用户/超时），仅在语句以错误收尾时归属取消；用户取消标「已取消：结果待核实」、同批次未发送的语句标「已取消：未执行」（原先静默丢弃）、超时标「已超时：结果待核实」。原先取消摘要直接透传 `[SQLSTATE 57014]`，既不提示取消也不能让历史判定未知。
  - **D3/D4 aborted 跳过**：补上增量一承诺但当时未落地的逻辑——语句回 `25P02` 置 `aborted`，`continue_on_error` 下后续非恢复语句本地标「已跳过：需 ROLLBACK 后继续」不再盲发，恢复语句照常下发、成功收尾后清除 `aborted`（不自动回滚，R27）。
  - 验证 — `pg_live_smoke_cancel_token_stops_long_query_promptly` 与 `pg_live_smoke_aborted_transaction_skips_remaining` 由失败转通过（其余 `pg_live_` 24/26 通过）；app 新增 `cancel_query_execution_reaches_the_executor` 等 3 测，`cargo test -p fluxdb-app` 422 通过。
  - **MySQL 停止（增量五末尾补）**：审计发现上一步只通了 PG —— MySQL executor 的 `execute_with_progress` 收到 `should_cancel` 但只在语句之间检查，`SELECT SLEEP` 这类单语句取消仍是死按钮。sqlx-mysql 不暴露连接 `CONNECTION_ID()`（握手后即弃，`#[allow(unused)]`），无 CancelToken API [R32 已核]，故按 `CONNECTION_ID()` → 另开短连接 `KILL QUERY <id>` 实现等效取消：执行每语句用 `tokio::select!`（语句 future vs 100ms 取消轮询），取消时 `KILL QUERY` 只中止当前语句、主连接保留；取消命中运行中语句标「已取消：结果待核实」、批次剩余标「已取消：未执行」。新增 `mysql_live_smoke_cancel_token_stops_long_query_promptly`（`SLEEP(120)` 取消 <20s 返回，`FLUXDB_MYSQL_SMOKE` 门控）。MySQL 无会话态（每次执行新拨一连接），故无 aborted 跳过需求。
  - 既存失败（与本次改动无关，`git stash` 基线同样失败）：`pg_build_table_ddl_round_trips_clauses`（生成列 DDL 重建）、`pg_create_database_sql_builds_options_and_quotes`（建库 SQL 少了 `TEMPLATE template0`）、`pg_live_smoke_table_info`（DDL 缺 `PRIMARY KEY ("id")`，同源）、`pg_live_smoke_create_delete_database`（容器残留 `t07_db`）。前两项属 `table_ddl`/建库渲染（T20/T12 范围）的未完成状态，未在本次处理。
...
- **完成记录**：进行中（FluxDB，2026-09-12）。
  ...
  **custom 格式未提供**（`BackupForm` 无格式选择器，恒为 plain .sql）——按设计条件项，**不实现 pg_restore**，不为此延误既定功能。
  ...
  - **修复记录三（2026-09-16，EDB macOS 包 pgAdmin framework 链接导致下载失败）**：实测 EDB 18.1-1 macOS 包下载报「不安全的客户端符号链接：../Python」。链条：精简轮校验不过 → 补齐轮（`include_optional=true`）按「末两段是 `lib/*.dylib`」把 `pgAdmin 4.app/.../Python.framework/Versions/3.13/lib/` 下的库也选进来，其中 `libpython3.13.dylib` 是跨目录符号链接（`../Python`），被 `write_symlink_entry` 的安全检查硬拒绝，整次安装报废。两层修复：① 条目选取按路径段排除含 `pgAdmin` / `*.framework` 的条目（framework 的 `libssl.3.dylib`/`libcrypto.3.dylib` 与真依赖同名会覆盖安装目录正确副本，本就不该收）；② `write_symlink_entry` 对不合规链接（绝对路径/跨目录/链式逃逸/指向其他链接）从返回错误改为 WARN + 跳过该条目，安全边界不变（非法链接一律不落盘），但不再连累整包安装。新增/更新单测 `pg_client_archive_skips_escaping_symlink_and_allows_library_alias`、`download_pg_client_tolerates_framework_symlinks_in_optional_round`、framework 条目过滤断言。
...
- **完成记录**：进行中（FluxDB，2026-09-12）。
  ...
  **增量六（本会话，补齐 F18 改密/重命名/角色选项 UI 入口）**：核对发现 T27 原实现只开放「新建/删除/成员/对象权限」，后端已支持的 `AlterPgRolePassword`/`RenamePgRole`/`alter_role_options`（dispatch 已有或连接器已实现）缺 UI 入口 → 补齐：
  - 改密/重命名：app `UserAdminState` 增 `pg_edit_mode: PgRoleEditMode`（None/Rename/Password）+ `pg_rename_new` + 4 个 AppCommand（BeginUserAdminPgRename / BeginUserAdminPgPassword / SetUserAdminPgRenameNew / EndUserAdminPgEdit）；desktop 工具栏为选中普通角色增「重命名/改密」按钮，内联编辑表单（重命名用新增 `user_admin_pg_rename_input`，改密复用既有 `user_admin_new_password_input`）；`pg_rename_role`/`pg_change_password` 走既有 `RenamePgRole`/`AlterPgRolePassword`。
  - 角色选项（可登录 LOGIN 切换）：app `UserAdminState` 增 `pg_role_login: BTreeMap<String,bool>`（LoadUserAdminUsers 时经 `load_pg_role_login_map` 填充）+ 新 AppCommand `SetPgRoleLogin`（dispatch 经 `alter_role_options(can_login=..)` 路由）；desktop 角色详情增「可登录 (LOGIN)」Switch（postgres/pg_* 只读，与删除一致）。
  验证 — `user_admin_pg_edit_mode_defaults_and_clears` 单测；真库 `pg_live_smoke_role_crud_and_membership` 扩展 LOGIN true→false 往返断言（alter_role_options→list_roles 读回一致）；workspace 全绿（app 401）；改密/重命名/角色选项真实链路均已真库覆盖。
...
| F09 查询/结果/进度/取消 | T13 `pg_live_smoke_cancel_token_stops_long_query_promptly`（`pg_sleep(120)` 取消 ~1.2s 返回并标「结果待核实」）、`pg_live_smoke_aborted_transaction_skips_remaining`（25P02 本地跳过 + 显式 ROLLBACK 恢复）、`pg_live_smoke_empty_result_retains_columns`；MySQL `mysql_live_smoke_cancel_token_stops_long_query_promptly`（`SLEEP(120)` 取消 `KILL QUERY`）；app `cancel_query_execution_reaches_the_executor`/`start_query_execution_resets_previous_cancel_flag`；结果标签 UI 待人工(C3/C4) | — | PG 真库已验证；MySQL 冒烟因本机无常驻 MySQL 容器（`FLUXDB_MYSQL_SMOKE` 门控、跑真库时执行）待补跑；「停止」按钮已接线，人工清单 D1–D5（PG）与 MySQL 停止待复测 |
| F10 格式化/参数/补全 | T12 分句：`split_sql_keeps_dollar_quoted_function_body_together`；T14 `pg_live_smoke_completion_metadata`/`_search_path_and_cross_schema`/`_cancel_returns_empty`/`_does_not_disturb_user_transaction`（跨 schema、重载签名、不影响用户事务）；T14 增量九（候选身份 + 详情取数 + 桌面回退收口）`postgres_completion_carries_schema_identity_for_documentation`（真库：`FROM tenant_a.order` 只出 `orders`、详情按身份出列）、`table_documentation_resolves_by_candidate_schema_identity`、`table_documentation_falls_back_to_persisted_snapshot`、`resolver_success_does_not_leak_local_fallback_candidates` | MySQL 补全原测试回归（connect 别名/CTE 通过） | 真库+单测；补全交互待人工(C1/C2、手册 E7) |
