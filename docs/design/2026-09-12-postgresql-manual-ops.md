...
| 步骤 | 操作 | 测试断言 |
|---|---|---|
| E1 | 新建查询，输入 `SELECT * FROM tenant_` 触发补全 | 按 search_path 顺序出 `tenant_a`、`tenant_b`；注释作文档提示 |
| E2 | 跨 schema：输入 `SELECT * FROM tenant_b.` | 出 `tenant_b` 的列，不混 `tenant_a` |
| E3 | 输入 `SELECT * FROM `OrderItems`` | 大写名正确引用，不折叠成小写 |
...
| 步骤 | 操作 | 测试断言 |
|---|---|---|
| G1 | PG 连接右键「用户与权限」 | 打开 PG 角色面板（非 MySQL user@host）；无 host/plugin/资源限制字段 |
| G2 | 新建角色 `test_r1`（LOGIN + 密码）与 `test_g1`（NOLOGIN） | 创建成功，列表刷新 |
| G3 | 把 `test_r1` 加入 `test_g1`（成员） | 关系正确；`test_r1` 右侧显示所属组 |
| G4 | 改 `test_r1` 密码 / 重命名 | 成功并提示；新密码可登录；完成后表单收起 |
| G5 | 新建态 → 关闭 `test_r1` 的「可登录 LOGIN」开关 | 变 NOLOGIN；刷新后开关反映 false |
| G6 | 对象权限：选种类=表、schema=`tenant_a`、对象=`orders` → 读取 | 显示属主；ACL 为 NULL 显示「属主 X（默认权限）」 |
| G7 | 对 `test_r1` 授予 SELECT（带/不带 GRANT OPTION）→ 刷新 | 该权限变「直接授权（可撤销）」 |
| G8 | 撤销该直接授权 | 仅撤销直接授权；继承/PUBLIC/属主显示不可直接撤销、无撤销按钮 |
| G9 ⚠️ | （P0-5 验证）对象权限面板函数授权：选种类=函数，签名填入 `int) TO postgres; ALTER ROLE test_r1 SUPERUSER; --` | **被拒绝**（弹「函数签名不合法」），绝不进 GRANT；填 `integer, text` 类正常签名则放行 |

失败判定：G9 恶意签名能提交 = P0-5 未生效（严重安全问题，立即停）。
