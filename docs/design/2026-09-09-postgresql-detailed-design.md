...
> **修复记录（2026-09-14，app 数据展示页筛选/排序 SQL 预览与表头排序字段）**：PG 数据展示页「应用筛选 & 排序」的预览 SQL 出现 `SELECT * FROM \`fluxdb_manual\`.\`tenant_a\`.\`orders\` ORDER BY \`id PK\` DESC` 两处方言/字段错误。
> 1. **预览 SQL 硬编码 MySQL 引用**：`filter_ui/panel.rs::data_editor_sql_preview` 的三处（`sql_qualified_object_name`、ORDER BY 的 `sql_quote_ident`、WHERE 的 `data_filter_rules_sql`）及 `sql_parser.rs` 的 `data_filter_rule_sql`/`data_sort_rules_text` 一律写死 `DatabaseKind::MySql`，PG 下仍生成反引号。修复：整体注入真实 `db_kind` —— 预览入口从 `this.connection_database_kind(editor.object.connection_id)` 取，Builder→Text 同步（`sync_data_filter_builder_to_text`、表头排序 `perform_data_table_header_sort`）经新增 `NavicatMain::data_editor_database_kind(tab_id)` 取该 tab 连接的方言。PG 走 `sql_quote_ident`/`sql_qualified_object_name` 既有双引号 + `"schema"."table"` 分支，MySQL 保持反引号。`data_filter_rule_text_signature` 仅为文本往返一致性比对，固定 MySql 引用不变。
> 2. **排序字段误用展示标题**：表头排序回调 `data_editor_model/table_delegate.rs::render_th` 用 `column.name`（`Column.name` 是展示标题，主键带 `  PK` 后缀）作为 SortRule.field，传入 connector 后在 `data_order_by_clause` 匹配不到列，POST 到 PG 的 `ORDER BY` 静默丢失，预览却把 `id PK` 原样拼进 SQL。修复：改用 `column.key`（真实字段名）；复制列名、列操作同源同步修正。
> 3. **方言差异收敛为统一 `SqlDialect` trait（2026-09-14）**：预览 SQL 的「标识符引用 / 对象名限定 / 分页子句」三处方言差异此前散落在 `filter_ui/sql_parser.rs` 的 `sql_quote_ident`/`sql_qualified_object_name`（内部 `db_kind == DatabaseKind::Postgres` 分支）与 `panel.rs` 的 `LIMIT/OFFSET` 拼接。本次在 **fluxdb-core**（`parts/sql_dialect.rs`，非 UI 渲染层）收敛为统一接口 `trait SqlDialect { quote_identifier / qualified_object_name / render_limit_offset }`，由 `sql_dialect(kind)` 按 `DatabaseKind` 解析（PG 双引号 `"schema"."table"`、MySQL/TiDB 反引号 `\`db\`.\`schema\`.\`table\``、SQLite 双引号 `"db"."table"`）。UI 仅持有 `ObjectPath`/`FilterSpec`/`SortSpec`/分页参数/`DatabaseKind`，`sql_quote_ident`/`sql_qualified_object_name`/分页子句改为委托方言对象，不再直接判断引号规则。转义规则与 connector 既有 `pg_quote_identifier`/`mysql_quote_identifier`/`sqlite_quote_identifier` 一致（本模块只服务预览生成，未改 connector 执行路径）。
> 4. **双引号文本往返**：筛选/排序文本解析器同步支持 PostgreSQL 双引号标识符及 `""` 转义；顶层关键字和列表切分会跳过双引号内部内容，避免字段名中的 `AND`、逗号等被误拆；签名比较统一归一化反引号和双引号。
> 5. **实际结果按源列类型排序**：PG 数据页为统一解码会生成 `"id"::text AS "id"` 一类投影；原先 `ORDER BY "id"` 会优先命中同名文本输出别名，使整数按字典序排列，表现为 `999` 排在 `5000` 前。数据页及一致快照导出统一给源表增加 `"__fluxdb_source"` 别名，排序与主键稳定排序明确引用 `"__fluxdb_source"."id"`，保持数据库真实类型语义。
>    - **MySQL 同根因修复（2026-09-14）**：MySQL 数据页同样把整数 `CAST(\`id\` AS CHAR) AS \`id\`` 投影成文本并沿用原列名，`ORDER BY \`id\`` 也会命中同名 CHAR 输出别名而字典序错排。数据页 `mysql_load_data` 给源表追加 `` `__fluxdb_source` `` 别名，新增 `mysql_order_by_clause` 用 `mysql_data_source_column`（源别名限定）构建用户排序与主键 tie breaker；WHERE 仍用未限定反引号列名（MySQL WHERE 不解析输出别名，无此问题）。回归：连接器单测 `mysql_order_by_clause_qualifies_source_column_to_avoid_text_alias_ordering` 覆盖源列限定、主键追加与反引号转义；预览导出走 `data_export_preview_select_list`（纯列名，无别名冲突）不受影响。
> 回归：desktop 单测 `data_editor_sql_preview_postgres_uses_double_quotes_and_real_column_names`（PG 普通/主键字段升降序 + schema/无 schema + MySQL 反引号保持）及 `postgres_filter_and_sort_text_round_trip_double_quoted_identifiers`（PG 筛选/排序 Builder→Text→Builder 往返）；表头/查询结果排序 `data_sort_rules_text` 各调用点补 `db_kind` 实参。连接器单测覆盖源列限定与标识符转义，真实 PG 冒烟用 `5000/999` 验证整数降序；方言 trait 覆盖测试：fluxdb-core `sql_dialect_tests`（PG/MySQL/SQLite 标识符转义、对象名限定、分页渲染四用例）。
...
- **增量九 候选身份与详情取数（缺陷修复）**：`SELECT * FROM tenant_a.order` 曾出现三个现象——详情面板报「对象不在索引或没有列」、混入关键字候选 `order`、混入 public 的 `OrderItems`。根因两条，均已修：
  - **候选身份全链路透传**：`QueryCompletionItem` / `fluxdb_editor_core::CompletionItem` / `DocumentationRequest` 与 `SqlDocumentationResolver` 各带一个 `schema` 字段，取值为**本次补全的 schema 作用域**（`completion_namespace_scope` 的结果，与写入 CompletionIndex 的桶键同一身份）；表/视图候选由 `table_completion_items` 落值。详情面板据此按 `(库, schema, 表)` 命中原子的完整身份，不再是恒定的 `schema=None`。旧的限定 label（跨 schema 同名消歧时的 `schema.table`）作为兜底拆出 schema。
  - **详情取数改为「索引优先 + 按需拉一次」**：内存索引 → 盘上快照 → 已打开数据编辑页的列 → 连接器（取到后写回索引与快照），复用列补全同一条链路。`tenant_a` 不在 search_path 时预热从不覆盖它，这条路是详情可用的唯一来源；取不到元数据来源时报「列元数据不可用：原因」，与「对象确实没有列」区分，不谎报成对象不存在。取消令牌改为编辑器共享原子（`CancellationToken::shared`），切换选中项时旧请求在建连/查询前即收敛。
  - **桌面本地回退候选收口**：`sql_editor_adapter` 原先把「整份方言关键字 + `with_schema` 注入的 schema 上下文」无条件并入 resolver 成功结果，导致关键字 `order` 与其它 schema 的表串进 `schema.` 的定向结果。现在成功路径只并入查询内语义候选（CTE/别名/派生列），本地回退只在 resolver 缺失/失败时使用（即该文件原本的设计意图）；成功但结果为空视为权威答案。

- **增量十 快照落盘取值域修复（既有缺陷）**：`TableFingerprint.fingerprint` 是 64 位哈希，此前按整数写 TOML，而 TOML 整数上限是 i64——只要某个表名/列名算出的哈希最高位为 1，`save_completion_index` 就整体失败（`out-of-range value for u64 type`），且调用方用 `let _ =` 吞掉错误，表现为「补全索引从不落盘、每次冷启动重查目录」。修法：指纹改为十六进制字符串序列化（`#[serde(with = "u64_hex")]`），`COMPLETION_INDEX_VERSION` 3→4（旧快照因版本不匹配被安全拒绝并重建）；快照读失败与落盘失败都改为可检索告警，不再静默。回归证据：`completion_index_round_trips_table_fingerprints_beyond_i64`（`u64::MAX` 指纹往返）、真库用例 `postgres_completion_carries_schema_identity_for_documentation` 末尾断言 `tenant_a` 作用域快照落盘且含真实列（去掉该 serde 编码即失败）。

验证证据（真实 PG 冒烟，`FLUXDB_PG_SMOKE` 门控）：`pg_live_smoke_completion_metadata`（表/列/批量列/例程/触发器 + 表列注释 + 引号建表 `"T14_Camel"`/`"Id"` 原名保留 + `t14_ovl(int)`/`t14_ovl(int,text)` 双重载签名）、`pg_live_smoke_completion_search_path_and_cross_schema`（默认 search_path 不含 t14_sa、档案默认 schema 生效、多段顺序 sa→sb、列取首个可见、显式跨 schema）、`pg_live_smoke_completion_does_not_disturb_user_transaction`（用户会话未提交数据在补全期间不被提交/回滚）、`pg_live_smoke_completion_cancel_returns_empty`。app 侧：跨 schema 同名表消歧、大小写不同对象不合并、重载分条与快照往返、DDL 后例程/触发器失效、元数据源不可用时降级保留本地候选。MySQL 回归：connectors 14 项、app 33 项通过；别名/CTE 相关 29 项通过。增量九新增证据：`postgres_completion_carries_schema_identity_for_documentation`（真库 `fluxdb_manual`：`SELECT * FROM tenant_a.order` 只出 `tenant_a.orders`、候选带 `schema=tenant_a`、详情按身份取出 `id/amount/note`、`schema=None` 时不伪造列）、`table_documentation_resolves_by_candidate_schema_identity` / `table_documentation_falls_back_to_persisted_snapshot` / `resolver_success_does_not_leak_local_fallback_candidates`。
...
实现记录（T16）：`app/parts/create_table_postgres.rs` 从领域结构生成 PG 语句计划——`CREATE TABLE` 内联列定义与联合主键、FK（`referenced_database` 承载 schema，**始终限定**，因为新建表所在 schema 常不在 search_path）、CHECK；表/列注释为独立 `COMMENT ON`；普通/唯一索引为独立 `CREATE [UNIQUE] INDEX`（BTREE 为默认不写 `USING`，FULLTEXT/SPATIAL 与索引前缀长度明确拒绝）；触发器为 `CREATE TRIGGER ... FOR EACH ROW EXECUTE FUNCTION "fn"()`，行内 `BEGIN…END` 体与 identity 挂非整数列均明确拒绝。provider 能力位关闭 MySQL 专属项（engine/charset/unsigned/zerofill/ON UPDATE/key length/binary），identity 复用 `auto_increment` 能力位但渲染为 `GENERATED BY DEFAULT AS IDENTITY`。`CreateTableState` 新增 `schema` 作用域，经 `OpenCreateTable` 与桌面入口下传（对象树带 schema、菜单入口取档案默认 schema），未指定时不写 schema 限定、交由连接 search_path 解析。
...
两条来自实机问题的硬约束：**条目选取排除 pgAdmin 与内嵌 framework**——macOS 官方包里 `pgAdmin 4.app/.../Python.framework/Versions/*/lib/` 下的动态库与客户端工具无关，且 framework 的 `libssl.3.dylib`/`libcrypto.3.dylib` 与真依赖同名会覆盖安装目录里的正确副本，`libpython3.13.dylib` 还是跨目录符号链接（`../Python`），因此补齐轮也按路径段排除 pgAdmin/`*.framework`，不能只按「末两段是 lib/*.dylib」匹配；**符号链接安全策略是跳过而非失败**——解压时链接目标只允许同目录文件名（拒绝绝对路径、跨目录、链式逃逸），不合规的链接条目只记 WARN 跳过、不落盘，绝不因单个条目让整次安装报废（官方包内合法的跨目录 framework 链接正是被此策略跳过的对象）。
