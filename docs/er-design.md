...
> 修订记录：  
> - 2026-09-21 逻辑关系表单修复：① 抽屉可拖宽（左缘手柄，宽度按 tab 记忆，280–560px，复用 SidebarResizeStart/Drag）；② 修复「选表后字段下拉恒空」——`flush_pending_columns` 字段加载结果同步回 `er_full_tables`，局部 ER 下选择不在图内表的字段也可选；③ 表单增加基数选择（1:1/1:N/N:1/N:N/未知），映射 `ErMatchCardinality`（用户选择 basis=UserAssertion，未知不声明），新建默未知、编辑预填可改。验证：desktop 432 全通过（含 `er_cardinality_option_roundtrip`）。
> - 2026-09-21 正确性修复 + 导入（本轮）：① 并发去重：`er_columns_core`/`er_relations_core` 缓存检查与 inflight 登记合并为单次加锁，8 线程并发关系加载合并为 1 次读库；② 取消链路：逐 key 取消旗标（`columns_cancel`）+ `AppController::er_columns_cancel`，关闭 ER 标签时取消该作用域在飞字段加载（逐表循环中间停止、完成丢弃结果）；③ 结构化身份贯穿字段加载：`ErColumnKey` 改持 `ErTableRef`，删除 `er_split_display`（`rsplit` 误拆含点表名），逐 schema 批读、按身份归并；④ 逻辑关系语义：`er_relationship_effective`（Confirmed+Current）过滤，画布只投影有效逻辑边，局部邻域纳入有效逻辑关系使逻辑孤邻可展开；⑤ ER JSON 导入解析/校验/差异/绑定（`er_import_parse`），先校验后应用、失败不改模型。验证：app 505 / core 104 / storage 34 / desktop 431 全通过。未做：结构快照重绑接入刷新（缺稳定标识 + 快照持久化）、导入「应用」编排、全自绘缩放。
> - 2026-09-21 实现结构快照重绑四步（§5.2，无第5步相似度猜测）：core `er_rebind.rs` `rebind_entity`（实体：稳定标识→限定名→unresolved；重建检测 stable 变但名同→needs_review；逐列：稳定→列名→unresolved，绝不静默接相似列）。app `ErModelService::rebind_report`（按旧实体映射+新快照判定每条关系两端实体/列，产出 unresolved 列/needs_review，不自动改关系，供 UI 列待处理项）。6+1 单测。验证：app 496 / core 104 / storage 34 全通过。
> - 2026-09-21 追加关系 JOIN 条件生成 + 生产存储适配链路：core `ErRelationship::join_plan`（仅 join_candidate 产出；`ErJoinPlan` 完整保留复合 column_pairs + required_filters，§6.6 不可截断；ON 条件语义，不拼任意 SQL）；app `ErModelService::join_plans`（批量、过滤不可用）+ `FileErRelationshipStore`（FileStorage→ErRelationshipStore 生产适配，打通 core↔service↔sqlite 全链路，重启读回一致集成测试）。验证：app 495 / core 98 / storage 34 全通过。
> - 2026-09-21 落地 D1-D9 逻辑关系**核心模型 + 持久化 + 服务**（地基，编辑 UI/导入/Agent 仍后续）：
>   - core `er_relationship.rs`（include! 进 crate root）：`ErRelationship`（revision / left-right entity / role(对内唯一) / 有序 column_pairs / structured required_filters / 双向基数+依据 / origin / review / enforcement / validity / evidence_refs）全部 serde；`ErUsage` + `ErUsageContext` + `ErRelationship::usage` 确定性判定（§5.9：incomplete→rejected→confirmation_outdated→stale/invalid→declared-constraint(禁未启用警告)→confirmed( logical_only/cardinality_unknown/fanout_risk )→confirmation_required）。6 单测。
>   - storage：`FileStorage::load_er_relationships/save_er_relationships`（kv `er_rel:{scope}`，全量 JSON upsert）。round-trip 单测（含跨作用域不串数据）。
>   - app `er_model_service.rs`（include! 进 crate root）：`ErRelationshipStore` trait（注入测试）+ `ErModelService`（create/update/confirm/reject/usage，create 校验+role 唯一，写操作 expected_revision 乐观锁；**定义编辑使旧确认失效并降级回 proposed**，否则 usage 的确认过期检查因 None 走不到 outdated 分支误判为候选）。6 单测（含编辑失效旧确认回归）。
>   - 验证：app 493 / storage 33(er_ 3) / core 96(er_relationship 7) / desktop 428 / connectors 225 全通过。
>   - 未做（依赖此地基继续）：逻辑关系编辑 UI、关系查询服务（搜索/路径）、导入、Agent/MCP。
> - 2026-09-20 实现 ER 视图状态持久化（§5.7/§十，分组折叠态持久化）：`fluxdb-storage` 增 `ErViewScopeState`（group/pinned/positions）+ `FileStorage::load_er_view_states/save_er_view_states`（sqlite `kv` 下 `er_views` key，JSON upsert）。desktop 打开 ER tab 时生成作用域 key（`connection_id:database:center_display`，按作用域唯一）并应用上次保存的分组/坐标/固定一次；分组变化、拖动结束、标签关闭时持久化。坐标/固定仅按展示名应用，表不匹配（改名/换库）多余项忽略、缺失表按布局回退；连接绑定在 key 内，不靠名称跨连接误绑。存储 round-trip 单测（跨实例重启读回一致）。验证：desktop 428 / storage 33 / app 487 / core 89 / connectors 225 全通过。
> - 2026-09-20 修复「ER 画布全空白」回归：搜索/小地图/导出 overlay 的包裹层 `div().relative().flex_1().min_h_0()` 未带 `.flex()`，使其内部 `flex_1` 子（`er_canvas_view`）在 block 布局下高度塌陷 → 探针测 0、不建场景 → 画布空白（工具栏/搜索/分组正常，仅画布区无图）。修：包裹层补 `.flex().flex_col()`，让 `er_canvas_view` 的 flex_1 在 flex 容器内填满。新增 taffy 回归 `er_canvas_overlay_wrapper_must_be_flex_container`（block 包裹 → 塌陷 0，能抓回归）。验证：desktop 428、app 487、core 89、storage 32、connectors 225 全通过。
> - 2026-09-20 实现 ER 导出（§7/§十）：工具栏「导出」→ JSON / DBML / Mermaid / SVG 写剪贴板。er/export.rs 纯函数（`er_export_mermaid/dbml/json/svg`），单测覆盖。JSON 带 `schema_version`（`ER_EXPORT_SCHEMA_VERSION`）、连接绑定、坐标/固定，不含密码；SVG 为展示型（表级连线简化，SVG 文本转义，标注有损）；Mermaid/DBML 表级关系按约束名去重。导入未做（依赖 D1-D9）。另补搜索键盘导航（§§五.6）：上下/Enter 定位、Esc 关闭、下拉高亮选中项。
> - 2026-09-20 修复复合外键约束身份（§6.2）：`ForeignKeyInfo` 增有序列对 `columns`/`ref_columns`（保持数据库原始序号），PG `pg_foreign_keys_from_structure` 不再把复合外键逗号拼接成不存在的单列「a, b」，改为保留有序列对 + 扁平首列兼容旧展示；ER 加载 `load_er_relations_from_db_with` 经 `er_expand_fk_columns` 按有序列对整个外键逐列展开成边（复用约束名作身份，同表对多条约束区分）。MySQL 本已逐列一行、不拼接。新增单测：复合外键按原始序号展开、单列/列数不匹配回退、PG structure 映射保留有序列对。验证：app 487 / connectors 225 / core 89 / storage 32 / desktop 138 全通过。
> - 2026-09-20 实施 ER 小地图（§4.3/§六.25）：右下角叠层，自定义 paint Element `ErMinimap`（`gpui::fill`/`quad` 画节点分布 + 当前视口矩形，>2000 节点采样有界，paint 回报窗口边界到 `er_minimap_rect`），`er_world_bbox` 统一世界包围盒，`er_minimap_center_world` 点击/拖动把世界点居中导航（保持缩放）。作为画布后置兄弟叠加（GPUI 无 z-index）。视觉需人工验收。
> - 2026-09-20 实施 ER 业务分组（§4.1/§4.2、§五.9-12）：按 schema 分组进入/返回全部。`er_group` 状态 + `er_group_bar` 分组 chips；进入组用纯函数 `er_group_subset_graph` 过滤展示图（表按结构化 `ErTableRef.schema`；边仅两端都属本组者=真实外键，不伪装聚合 JOIN），保留已有人工坐标、重建场景不丢布局，可任何时候回全部；孤立表始终可达可搜索。单测覆盖分组过滤与跨组边排除。
> - 2026-09-20 实施 ER 表搜索/定位（§3.2/§3.6、er-ui-relationship-canvas.md §6 搜索定位）：per-tab `Entity<InputState>`（惰性创建 + `cx.subscribe` 到 `InputEvent::Change` 更新查询字符串），工具栏下搜索条，按表名/注释包含查询（忽略大小写）过滤；命中下拉浮层作为画布后置兄弟绝对定位叠加（GPUI 无 z-index，后置者在上），点击命中表 `er_center_on_table` 保持当前缩放、平移使表居中并选中。`ErNodeMeta` 增 `comment`（搜索注释 + tooltip 完整说明）。纯命中判定提取 `er_search_text_matches` 单测覆盖。tab 关闭补 `er_search_*` 清理。未做：键盘上下选择/Enter 定位、关系列表、打开详情。
> - 2026-09-20 实施 ER 画布「视图变换缩放」（§2.3），并修复 tab 关闭状态泄漏。**取代旧 TODO「缩放需重写为全自绘」（GPUI div 无法 transform 文字）的判断**：经评估，本轮采用视图变换缩放——`ErViewport` 增 `scale`，绘制 `world*scale+pan+origin`、命中逆变换 `/scale`、节点拖动增量 `/scale`；卡片仍以固定屏幕尺寸渲染（不 transform 文字），非 1 缩放下卡片会重叠/分散，规避自绘整套文字的成本与风险。`zoom_around` 保证锚点下世界点静止（围绕光标/画布中心）；`visible_world_bounds` 除以 scale 使可见世界范围随缩放改变。工具栏加「缩小 / 百分比 / 放大 / 适配当前范围」（`er_fit_scope`：节点世界范围整体适配画布、居中、32px 边距，空图回起点）；Cmd(mac)/Ctrl(win/linux)（`modifiers.secondary()`）+ 滚轮围绕光标缩放。坐标统一收敛到 `ErViewport::to_screen_*/to_world_*`。新增回归：`er_zoom_transform_roundtrip_and_anchor_stable`、`er_zoom_out_reveals_more_nodes_in_materialize`。补 tab 关闭时 `er_canvas_origins/er_all_edges/er_center_refs/er_field_highlights/er_last_updated/er_refreshing` 清理（消除陈旧 TabId 状态泄漏）。验证：`cargo check --workspace`、desktop `cargo test` 136 er 用例通过、`cargo build -p fluxdb-desktop` 通过。未做：小地图/搜索/分组（相邻专项）；缩放级细节隐藏未做；Win/Linux 未实测。D1-D9 未改动。
...
> - 2026-09-20 实施「元数据分阶段加载 + 字段按需 + 局部 ER 复用缓存」（§3.4/§4.2）：表目录先出节点（固定槽位稳定网格，节点内滚动看全字段、不随字段数重排）；关系索引后台一次性读取并缓存，供整库与当前表关联 ER 共享复用；字段只按可见表+overscan 按需加载（去抖合并、app 层缓存/去重/连接修订失效，PG 跨 schema 同名表按 display 名隔离）。落地 core `ErLoadStatus` 状态（er_model.rs）、app 共享缓存与编排（er_catalog.rs `ErCatalogCache` + `er_relations_core`/`er_columns_core` + er_service 分阶段读取）、desktop 固定槽位渲染与按需提交（er/canvas.rs）。未做：分页表目录、PG 外键批量（仍逐表）、中途取消阻塞驱动、无界全局缓存。D1-D9 逻辑关系模型仍待确认。验证：`cargo test` app 470 / core / connectors / desktop 413 通过。
> - 2026-09-20 实施「ER 画布视觉与交互改造」（er-ui-redesign.md）：紧凑卡片 + 字段分列 + 真实高度 + 长表内部滚动 + 首次按宽度确定网格（槽位固定、卡片收短不移动他表）+ 表级端口圆角折线（改用折线包围盒裁剪，替换旧直线端点判定）+ 选择高亮（表头点击选择、相关连线高亮、空白/Esc 取消）+ 工具栏「回到起点 / 重新排列」。canvas.rs 按职责拆成 scene/node/interaction 三个文件（保留 include! 作用域）；滚动条改用像素偏移存储（不再浮点行累积）。几何/布局/折线/裁剪/滚动全部纯逻辑单测覆盖。D1-D9 逻辑关系模型仍待确认，未改动。
> - 2026-09-20 实施「关系画布第二阶段」（er-ui-relationship-canvas.md）：**取代旧固定列网格/仅表头端口/禁止单节点拖动**。关系驱动布局（app `er_layout.rs`：连通分量 + Tarjan SCC 环压缩 + 分层，被引用左/引用右，孤立表外围网格，确定不重叠无截断）；字段级端口（接真实字段行中心，随滚动移动，离屏用 ↑N/↓N 汇总端口点击定位，未加载用「字段待加载」汇总端口，找不到不接同名近似字段）；自由坐标 + 表头拖动单表（4px 阈值，未移动=选择）；首次关系就绪且用户未操作自动应用一次布局，已操作保留坐标 + 「按关系排列」按钮；5 色低饱和主题调色板按分量取色；正交圆角折线 + 关系线 hover/点击显示 `表.字段→表.字段`；基数无可靠唯一性元数据故保持中性不显示 1/N。坐标/滚动/选中/固定 与 拓扑分离（按表名），渲染期网格索引物化。验证：desktop `cargo test` 411、app `cargo test` 477（新增 er_layout 7 用例）、core/connectors/storage 全通过。D1-D9 逻辑模型仍待确认，未改动。
> - 2026-09-20 接管回顾修复：修复被误放到 crate root 的关系画布回归测试模块（移回 `mod tests`）；修复折线 base 底稿非正交（`(a_exit,a.y)→(b_enter,b.y)` y 不同产生斜线 → 改为水平出→竖直降→水平进）；修复 `route_field_edge` 预算用尽静默接受穿卡兜底 → 改为有界外侧绕行（卡左右外侧车道 + 障碍上/下绕行带，命中合法即返回，否则碰撞最少，不无限搜索）；修复 app `er_layout.rs` 分层 `level` 计算（多子 scc 用固定 `cur` 逐个比较，较浅子值覆盖较深值 → 引用两层分支被压成同层竖柱；改用运行取最大）。新增回归用例：真实 demo 六表链左→右层次 + 孤立表外围 + 真实卡片矩形两两不相交、折线全正交、拖动障碍进间隙后重路由不穿卡。基准（纯 CPU）：关系布局 50→1ms/200→1.6ms/1843→18ms/10000→108ms，场景构建 0.4→18ms，本帧物化(网格+路由) 0.36→7.2ms（每次 render 同步执行，为已知热点，见文档时效说明）。验证：desktop `cargo test` 132 er 用例（含 3 忽略的 perf 探针）、app 482、core 89 全通过。D1-D9 逻辑模型仍待确认，未改动。

> - 2026-09-20 排查与修复（本轮接管阶段）：锁定并修复第 §二 条审查清单及其它正确性问题，并补充只读 ER 加载/刷新闭环：
>   - 结构化表身份：新增 core `ErTableRef(database, schema, name)`，`ErTableNode.reference` 与 `ErForeignKeyEdge.from_reference/to_reference` 统一承载（er_model.rs）。所有邻域/布局/场景匹配改按身份进行，正确处理 PG 跨 schema 同名表与含 `.` 等合法标识符，不再用展示名或 `rsplit('.')` 字符串特例。`OpenErDiagram` 的中心表改为结构化身份（dispatch.rs），修复 PG 中心表因裸名/ `schema.table` 不匹配导致空图。邻域计算（er_service `er_neighborhood_included_tables`）按 `ErTableRef` 匹配，不再遗漏跨 schema 入向/出向邻居。新增 cross-schema + 含点标识符回归用例。
>   - 局部 ER 深度切换/单节点展开不再撕空图、不再丢人工坐标：新增桌面 `recompute_local_er_graph`/`recompute_local_er_from_center` 与 `er_all_edges`/`er_center_refs`，边集就绪时同步重算邻域（保留 `er_scene_positions`/`er_pinned`/滚动/视口），避免空画布窗口；边集未就绪才回退触发后台读取。
>   - 重试真正重新查询：新增 `er_columns_invalidate_failed` / `er_relations_invalidate_failed` / `er_columns_retry_core` / `er_relations_retry_core`（app 层，范围明确、保留成功结果、不每帧自动重试）。字段失败节点“重试”与工具栏“重试关系”都先作废 Failed 缓存再重新读库；连接器实际被重新调用（回归用例验证读取次数）。
>   - 外键标记语义：只把真正持有外键的 from 端字段标 FK；被引用(to)端不是自身 FK（除非它也是某边 from 端）。PK/FK 并存图标与 tooltip 准确。
>   - 关系线命中校正：空白点击命中测试补上画布原点偏移（`world = screen - origin - pan`，与绘制 `world + pan + origin` 一致），覆盖侧栏/工具栏/窗口尺寸与不同 DPI。
>   - 只读 ER 刷新闭环：工具栏“刷新元数据（保留布局）”重读表目录+关系，保留当前可用图、视口与人工坐标/固定；“更新时间”与“刷新中…”状态反馈；空库与正常加载分开明确提示（“该数据库暂无表”+刷新入口），不再混为同一空白画布。缓存仍按 `(ConnectionId,连接修订,database,schema)` 分区、修订变化整体失效。
>   - 隐藏字段精确定位：新增 `hidden_field_scroll`（目标字段居中进入可视区，非仅滚顶/滚底）+ `er_field_highlights` 行高亮；滚动与高亮在空白/Esc 后清除。多字段的字段及关联目标映射（`ErNodeMeta.field_targets`）已建立，供后续定位菜单复用。
>   - 验证：`cargo test` core 89 / app 485 / desktop 419 全通过（新增 cross-schema、列/关系重试、隐藏字段精确定位等回归）；`cargo run -p fluxdb-desktop` 编译并启动到单实例守卫。D1-D9 逻辑关系模型仍未实施，未做缩放/小地图/分组/持久化/导入导出单独专项。
>
...
- **当前表关联 ER**：表右键菜单「关联 ER」→ 同一 `OpenErDiagram`（表级 path）→ `表名 · 关联 ER` 标签；`ErDiagramState.center_table` 标记中心表，`load_er_neighborhood_in_background` 以中心表 1 跳（入向/出向外键）过滤子图后走同一画布。<br>**附加（性能方向）**：FK 批量化（Connector `list_foreign_keys_for_tables`，MySQL 单次查全库）+ 画布二维虚拟化（视口+overscan 内才创建节点控件，节点行索引二分 + 完整 x/y 相交，连线 y 带索引 + 精确线段相交，场景后台预计算）+ 拖动平移（`ErViewport.pan`，重载保留不跳回原点）。曾实现的截断（前 300 张）与首字母分组方案均已被虚拟化替代并删除。缩放留 §13 TODO。
- **验证**：`cargo fmt`、`cargo check --workspace`、（app）`cargo test` 463、（desktop）`cargo test` 410 全部通过（含 `load_er_graph_reads_tables_and_foreign_keys`、`load_er_graph_reads_demo_database`、`er_visible_nodes_and_edges_cull_to_viewport`、`er_node_intersect_touching_and_negative_pan`、`er_edge_exact_intersect_cases`、`er_edge_index_bounds_query_work_with_many_edges`、`er_scene_handles_10k_tables`、`load_er_neighborhood_keeps_center_and_one_hop`）、desktop 可 `cargo build` 与 `cargo run`（GUI 命中单实例守卫、主窗口可启动）；macOS 实机验证。
...
## 12b. 实施记录：画布视觉与交互改造（2026-09-20，er-ui-redesign.md）

按 [er-ui-redesign.md](er-ui-redesign.md) 实施紧凑卡片视觉、稳定网格、表级折线与选择交互。**明确取代前一阶段「所有可见卡片必须等高」约定**；保留二维虚拟化、分阶段加载与字段按需读取，不退回全量加载。本轮不扩展缩放/小地图/关系编辑/搜索侧栏，不引入 WebView，未改动 D1-D9 确认状态。

落地与范围（desktop 侧，`apps/fluxdb-desktop/src/main_parts/er/`）：

- **职责拆分**：原 `canvas.rs`（>1400 行）按职责拆为
  - `canvas.rs`：加载编排（表目录→列数确定→字段按需→关系索引）与画布/工具栏/状态装配；
  - `scene.rs`：几何/布局/折线路由/空间索引/裁剪（`ErScene`/`ErEdge`/`ErViewport`）；
  - `node.rs`：卡片渲染（强调条/表头/三列字段/页脚/骨架/失败/滚动）；
  - `interaction.rs`：画布视口（探针/拖动/键盘/选择）与连线绘制（`ErCanvas`）。
  保留 `include!` 作用域（main.rs 顺序 include! 四个文件到同一模块）。结构拆分与行为修改尽量分离；加载/关系/字段/相邻服务的编排行为保持原语义。
- **卡片与字段**：槽位固定（宽 260、间距 X72/Y48、槽位高 256=最大卡片），卡片左上对齐槽位、外框按实际内容收短（`card_height` 与绘制同源）；宽度首次测量后确定列数 `max(1,floor((w-64+72)/(260+72)))` 且不超过表数；字段行三列（图标槽 14px + 名称 + 右侧 88px 类型列），名称/类型独立数据（弃用拼接字符串）；主键 key 图标、外键 link 图标（新增 `AppIcon::Key`/`AppIcon::Link` 与 `key.svg`/`link.svg`，FK 只按已加载真实边回填，不外推）；长表（>8 行）固定可视区内部滚动，按像素偏移存 `er_node_scroll_px`（按 tab+表名，虚拟化移出/移入恢复，刷新字段变少 clamp 不越界），右侧细滚动条可拖拽；页脚显示 `1–8 / N 个字段`。
- **稳定布局**：列数在首次测到画布宽度后确定，之后 resize 只更新裁剪与可见集合、不自动重排；「重新排列」按钮按当前宽度重算列数并回到起点，不清字段/关系缓存；字段/关系到达只重建受影响展示数据，不清 pan、不移动其他表（有单测）。
- **表级折线**：端口位于表头区左右边界、y=强调条下表头中点（不随字段/滚动变化）；正交折线拐角 6px 圆角；同排相邻走两卡空隙、同排隔表绕行间通道、跨行走列间通道、自关联绕出可见回路；`ErEdge` 由「两端直线」改为携带折线顶点+保守包围盒，可见性查询改用包围盒判定（两端屏外但折线穿视口会保留；不再按旧直线端点判断可见性）；边 y 带索引与长边处理保留。选中表时相关连线加粗+强调色、其余较弱。
- **选择与交互**：点击表头选择表（卡片强调色轮廓，不改变尺寸）；空白/Esc 取消选择；选中不触发数据库重读、不重置字段滚动；当前表 ER 改为表头右侧「展开关联」图标按钮（保留逐层展开/单节点展开）；点阵背景按 spec 3.1 暂用纯色（`UiColors.canvas_bg` 新增 token），未启用点阵以免帧开销。
- **滚动条组件说明**：`gpui-component` 滚动条要求持久 `ScrollHandle` 绑定 Stateful 元素，而本画布节点随二维虚拟化创建/销毁，无法跨重进入保留句柄；故按 spec 4.4 检查后采用轻量本地滚动条（像素滚动 + 可拖拽细条）。

验证：`cargo fmt`、`cargo check --workspace`、desktop `cargo test` **416 通过**（含新的高度/滚动/布局稳定/折线各路由与裁剪/万表/自关联/带索引等单测），app `cargo test`（含 ER catalog）通过。仅 macOS 实机验证；启动成功与单实例守卫命中不等同 ER 页面验收。开放式：键盘字段导航（PageUp/Down/↑/↓ 接线在画布 on_key_down，需画布持有焦点，交付前未实机验证）、Windows/Linux 三端口。

## 12c. 实施记录：关系画布第二阶段（2026-09-20，er-ui-relationship-canvas.md）

按 [er-ui-relationship-canvas.md](er-ui-relationship-canvas.md) 实施关系驱动布局、字段级端口、单节点拖动与关系反馈。**明确取代 12b（旧 er-ui-redesign.md）的固定列网格、仅表头端口、禁止单节点拖动三项限制**。验收目标是「字段沿线读懂关系、表位置体现关联、可拖动单表、保留大库性能」。

落地（desktop `apps/fluxdb-desktop/src/main_parts/er/` + app `crates/fluxdb-app/src/parts/er_layout.rs`）：

- **关系驱动布局（app 层，纯逻辑无 GPUI）** [er_layout.rs](../crates/fluxdb-app/src/parts/er_layout.rs)：由真实外键边计算连通分量；每分量用 Tarjan SCC 压缩环成 DAG，再最长路径分层（被引用方居左、引用方居右）；SCC 内节点同层垂直排布；孤立表放外围紧凑网格。输出确定、非重叠、无截断（50/200/1843/10000 表测试覆盖）。布局在后台执行，UI 不做力导向/每帧重排。
- **坐标/拓扑分离** [scene.rs](../apps/fluxdb-desktop/src/main_parts/er/scene.rs)：场景只存不可变拓扑与展示列（`ErScene`，Send 可后台构建）；坐标/滚动/选中/固定为桌面可变状态（按表名，稳定表身份）。渲染期 `materialize` 用自由矩形网格索引（`ErNodeGrid`，支持负坐标）查询可见节点；边按两端卡片联合包围盒保守筛选后解析字段锚点并路由（只处理可见边，两端屏外但路径穿视口保留）。
- **字段级端口**：锚点由节点世界坐标、表头高度、字段 padding、真实字段顺序与当前滚动共同计算（§4.1）。字段可见 → 接到真实字段行中心（半行滚动随动）；字段被滚出可视区 → 上/下汇总端口（小方块徽标 `↑N/↓N`，点击滚回定位，只统计隐藏且有关联的不同字段数）；字段未加载 → 表头「字段待加载」汇总端口；字段加载后仍找不到 → 表头端口 + tooltip 说明未解析，绝不静默接同名近似字段。整卡在画布外不改变真实端点，也不吸到屏幕边缘。
- **自由坐标 + 单节点拖动** [interaction.rs](../apps/fluxdb-desktop/src/main_parts/er/interaction.rs)：表头按住拖动单表（4px 阈值，未移动释放=选择）；空白拖动平移画布；字段滚轮只滚字段、滚动条拖拽只调滚动、徽标/按钮/展开不穿透。拖动更新 `er_scene_positions` + notify，连线端点/邻接边随坐标每帧重解析（只动受影响几何，不重建拓扑、不每帧全图布局）。画布外释放/切标签/关标签清理拖动状态。用户坐标优先，不偷偷弹回。
- **一次性关系布局应用（§6.2）** [canvas.rs](../apps/fluxdb-desktop/src/main_parts/er/canvas.rs)：表目录先到 → 临时概览排列（可操作）；首次关系就绪且用户尚未操作时自动应用一次关系布局；已操作则保留坐标 + 工具栏提示「可按钮按关系排列」。字段/关系变更只重建拓扑（坐标保留），不自动重排不清缓存。「回到起点」回初始视口；「按关系排列」保留手动固定节点；「重置手动位置并排列」显式清固定。手动拖动过的表入 `er_pinned`，重排保留其坐标。
- **配色与关系反馈**：`er_palette` 5 色低饱和主题调色板（蓝/青绿/紫/橙/玫红，明暗自适应），按连通分量序号稳定分配（孤立表/关系未就绪用默认蓝）；顶部 4px 色带 + 选中轮廓 + 关联连线高亮（2px，其余 1.25px）。关系线正交圆角折线（7px 拐角），hover/点击关系线显示 `源表.字段 → 目标表.字段` 说明；同表对不同约束独立不合并；自关联绕表外侧成可见回路；并行边稳定通道错开。**基数：现有 `ErForeignKeyEdge`/Connector 元数据无可靠唯一性约束，故保持中性端点，不显示 1/N（未知不猜，§5.2）**；复合关系字段配对各端点独立保留。
- **命中测试**：空白点击先只扫本帧可见折线的 `er_frame_edges`（有界）判定关系线，命中则显示说明，否则取消选择并开始平移；hover/选中/滚动走空间查询，不逐次全量扫描所有节点/字段/边。

验证：`cargo fmt`、`cargo check --workspace`、desktop `cargo test` **411 通过**、app `cargo test` **477 通过**（新增 `er_layout` 7 用例）、core/connectors/storage 全通过。性能对比同构型/同数据/同视口（见 §13 下方记录）。**未做**：缩放/小地图/业务分组框/关系编辑/布局持久化到磁盘（单标签生命周期保留坐标）/D1–D9；基数 1/N 未启用（缺唯一性元数据）；点阵背景仍用纯色。开放项：键盘字段导航需画布持焦点未实机验证、Windows/Linux 三端口、大库连续平移 FPS 待用户实测。
...
- [x] **画布二维虚拟化（替代截断/分组方案）**：全库 ER 直接进入同一张画布，全部表都有布局位置、可平移访问。渲染只对「视口 + OVERSCAN(200px)」内的节点创建控件、连线经索引与精确相交后交绘制层。节点：`ErScene::visible_nodes` 先在行范围索引上 `partition_point` 二分跳过视口前的行，再逐节点做完整 x/y 矩形相交——避免「只按行最大高度筛 y、之后只判 x」导致同排已离开视口的矮表在超高表行里被误挂载。连线：`build_er_scene` 按静态直线布局在后台建 y 带索引（`edge_bands`，世界 y 等宽带，边按 bbox y 跨度入带）；`ErScene::visible_edges` 平移时只扫与视口 y 重叠的少数带得到候选边，再做 x AABB 快速拒绝 + Liang-Barsky 精确线段相交，两端屏外但线段穿视口的线保留、bbox 相交但斜线未穿视口的假阳性剔除、不为未穿视口的长斜线构建路径。场景（布局/行范围索引/边索引/连线端点）在后台线程一次构建、平移不重建不深拷贝全图。画布真实尺寸由 `ErCanvasProbe` 回报（首帧用明确估算值，测量到位后重算可见集合，尺寸变化才 notify）。**画布空白根因修复**：`er_canvas_view` 内层视口 div 是 block 且其子元素全为 absolute，若父容器（外层 div）不是 flex_col，block 布局会把内层视口高度塌陷为 0（`overflow_hidden` 裁掉全部节点/连线，探针测 0 又忽略 → 永远空白）。修复：外层 div 恢复 `.flex().flex_col()`，内层 `flex_1.min_h_0()` 作为 flex item 填满工具栏下方剩余区域。用 gpui 编译的 taffy 0.13 引擎复现：修复前内层 1200×0、修复后 1200×764。新增 `er_canvas_layout_fills_below_toolbar` / `er_canvas_layout_empty_graph_does_not_collapse` 布局回归测试（dev-dependency `taffy=0.13`，与 gpui 同版本已锁定）。拖到画布外松开鼠标由 move 事件检测左键释放，拖动状态不卡住。**首字母分组概览与截断（前 300 张）均已移除。**测试：1843 表（1730 张 S 前缀）、10,000 表下可见节点数有界，节点相交（同排超高/矮表、边界相切、负/远视口）、线段精确相交（两端屏外穿视口、bbox 相交但斜线未穿、水平/垂直/零长度/贴界）均有覆盖；带 4000 条 FK 边（含大量远距外键）验证 y 带索引候选量 393 ≪ 全表边 4000、查询 235µs。
- [x] **拖动平移**：空白处按住拖动平移画布，节点 div 与连线 canvas 共用 `ErViewport.pan`；单节点展开/深度切换重载保留平移（不跳回原点，修 bug）。
- [x] **缩放**（design §2.3）：本轮改为**视图变换缩放**（不重写全自绘）。`ErViewport` 增 `scale`，绘制/命中/拖动统一 `world*scale+pan+origin`；`zoom_around` 锚点世界点静止；工具栏 +/-/百分比/适配当前范围（`er_fit_scope`）；Cmd/Ctrl+滚轮围绕光标。卡片固定尺寸，非 1 缩放会重叠（视图变换取舍，见修订记录）。适配「缩放围绕锚点、命中一致」。
- [x] **渲染缓存**：`ErScene` 预计算布局/行索引/连线端点，随图加载一次构建、渲染复用；加载与场景构建均在后台线程。
- [x] **字段按需加载**：仅当前可见表+overscan 的字段按需批量读取（去抖合并、app 层缓存去重、连接修订失效）；本视觉改造保留该机制，不退回全量挂载。
- [x] **列数按宽度确定 + 槽位稳定**（er-ui-redesign.md §5）：首次测到画布宽度后确定列数（不再固定 5 列），后续 resize 不自动重排；槽位固定、卡片按内容收短不移动其他表；「回到起点 / 重新排列」工具栏。
- [x] **长表内部像素滚动**：固定可视区（8 行）+ 虚拟行，按像素偏移存 `er_node_scroll_px`（按 tab+表名，移出/移入恢复，刷新字段变少 clamp 不越界），右侧细滚动条可拖拽；滚动条使用本地轻量实现（gpui-component 需持久 ScrollHandle，与节点虚拟化冲突，见 §12b）。
- [x] **表级端口圆角折线**：`ErEdge` 由两端直线改为折线顶点+保守包围盒；可见性查询改用折线包围盒（替换旧直线端点判定），保留 y 带索引与长边处理；同排隔表绕行、跨行走列间通道、自关联可见回路。
- [x] **选择高亮**：表头点击选择表（强调色轮廓），相关连线高亮、其余较弱；空白/Esc 取消选择；选中不重读、不重置字段滚动。
- [ ] **点阵背景**（er-ui-redesign.md §3.1）：本轮用 `UiColors.canvas_bg` 纯色画布；点阵随世界坐标平移、单层绘制视口内，需实测帧开销后决定启用。
